...
def test_slice_axis_0d():
    ...
    enc = ef.slice_axis(torch.from_numpy(x.asnumpy()), axis=0, begin=1, end=3)
...
def test_slice_axis_1d():
    ...
    enc = ef.slice_axis(torch.from_numpy(x.asnumpy()), axis=1, begin=0, end=2)
...
def test_slice_axis_2d():
    ...
    enc = ef.slice_axis(torch.from_numpy(x.asnumpy()), axis=1, begin=0, end=-4).numpy().astype("float32")
...
def test_slice_axis_3d():
    ...
    enc = ef.slice_axis(torch.from_numpy(x.asnumpy()), axis=2, begin=0, end=-4).numpy().astype("float32")
