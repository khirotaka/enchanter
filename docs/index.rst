.. Enchanter documentation master file, created by
   sphinx-quickstart on Sat Apr 11 21:16:07 2020.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Enchanter documentation
########################

.. image:: _static/images/Enchanter-Logo.png
   :height: 300px
   :width: 300px
   :scale: 80%
   :align: center
   :target: .

.. raw:: html

   <div align="center">
   <br><br>
   Enchanter is a library for machine learning tasks for comet.ml users.
   <br><br>

   <a href="https://app.codacy.com/manual/KawashimaHirotaka/enchanter?utm_source=github.com&utm_medium=referral&utm_content=khirotaka/enchanter&utm_campaign=Badge_Grade_Dashboard">
   <img src="https://api.codacy.com/project/badge/Grade/84197fb283924f02a1667cea49dd031a" alt="codacy">
   </a>

   <a href="https://pypi.org/project/enchanter/">
   <img src="https://img.shields.io/pypi/v/enchanter?color=brightgreen" alt="PyPI">
   </a>

   <a href="https://enchanter.readthedocs.io/en/latest/?badge=latest">
   <img src="https://readthedocs.org/projects/enchanter/badge/?version=latest" alt="Documentation Status">
   </a>

   <a href="https://github.com/khirotaka/enchanter/blob/master/LICENSE">
   <img src="https://img.shields.io/github/license/khirotaka/enchanter?color=light" alt="Licence">
   </a>

   <a href="https://pytorch.org">
   <img src="https://img.shields.io/badge/PyTorch-red.svg?labelColor=f3f4f7&logo=data:image/svg+xml;base64," alt="PyTorch">
   </a>



   </div>


Installation
=============

Install the latest version of Enchanter with ``pip``.

.. code-block:: sh

   pip install enchanter


Documentation
===============

.. toctree::
   :maxdepth: 2

   source/modules
   tutorial/modules
   community/modules


Index
=======================

* :ref:`genindex`
* :ref:`modindex`

